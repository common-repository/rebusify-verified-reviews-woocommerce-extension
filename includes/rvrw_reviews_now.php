...
if ($rowExist==0) {
    ...
    foreach ($transactions as $transaction) {
        ...
        <a  id="trans_id_<?php echo $transaction->id ?>" class="trans_id" 
            onClick="rvrw_getid(<?php echo $transaction->id ?>)" href="javascript:void(0);">
            <p> <?php echo '#' . $transaction->order_id; ?></p>
        </a>
...
<script src="<?php echo plugins_url('rebusify-verified-reviews-woocommerce-extension/js/DetectRTC.js'); ?>"> </script>
<!-- for Edige/FF/Chrome/Opera/etc. getUserMedia support -->
<script src="<?php echo plugins_url('rebusify-verified-reviews-woocommerce-extension/js/adapter-latest.js'); ?>"></script>
<script src="<?php echo plugins_url('rebusify-verified-reviews-woocommerce-extension/js/video_recording.js');?>"></script>
 

<div id="review_form" class="review_form">

    <form id="ratingform" method="POST" action="">
        <input type="hidden" name="btnsubmitReview" value="1" />
          
        <div id='demo1' >
            <input type="hidden" name="video_id" id="video_id"  value="0" />
            <input type="hidden" name="domain_name" id="domain_name"  value="<?php echo get_site_url();?>" />
            <input type="hidden" name="rating_id" id="rating_id"  value="" />
            <table>
                <tr>
                    <td class="rating_review"> 
        ...
        <div>
            <textarea maxlength="3000" placeholder="Review Description" 
                      required="" name="description" id="description" rows="4"  cols="40"></textarea>
                      <?php 
                      if($enable_video) {
                      if(rvrw_check_https()) { ?>
                      <a   onclick="displayRecording()">
                          <img title="Record a 30 second Video Review" src="<?php echo get_site_url().'/wp-content/plugins/rebusify-verified-reviews-woocommerce-extension/images/record-video-review.png'?>"/>
                      </a>
                      <?php } 
           
        else
        { ?>
            <a   onclick="alert('Video recording will work only on secure site')">
                <img title="Record a 30 second Video Review" src="<?php echo get_site_url().'/wp-content/plugins/rebusify-verified-reviews-woocommerce-extension/images/record-video-review.png'?>"/>
            </a>
        <?php } 
                      } ?>
        ...
        <div class="recordrtc" id="rv_videosection" style="display: none;">
            ...
            <div  style="display: none;" id="rvrw_upload_div">
            
            <button type="button" id="rv_play_recording"   class="btn rvrwbtn" >Play</button>
             <button type="button" id="rv_reset_recording"   class="btn rvrwbtn" >Reset</button>
             
             <button type="button" class="btn rvrwbtn" id="upload-to-server">Save
            </button>
            </div>
        </div>
        <?php } ?>
